...
# from system_users.forms import SUserPasswordChangeForm, SUserChangeForm, AdminPasswordChangeForm
# from system_users.models import SUserPassword, SUserSecurityQuestion, ExtendedSUserPermission, SUser, Role, Permission, \
# 	RolePermission


# class SUserPasswordInlineAdmin(admin.StackedInline):
# 	"""
# 	This class registers SUserPassword model with backend Admin.
...
# 	form = SUserPasswordChangeForm

# 	def get_queryset(self, request):
# 		"""Limit the queryset for passwords to the last 3"""
# 		qs = super(SUserPasswordInlineAdmin, self).get_queryset(request)
...
# 			except Exception:
# 				pass
# 		if SUserPassword.objects.filter(SUser__id=request_id).exists():
# 			return False
# 		return True
...
# class SUserSecurityQuestionInlineAdmin(admin.TabularInline):
# 	"""
# 	This class registers SUserSecurityQuestion model with backend Admin.
# 	This is a stacked inline admin form, so, do we register here? Of course not :-*
# 	"""
# 	model = SUserSecurityQuestion

# 	extra = 1

# 	def get_queryset(self, request):
# 		"""Limit the queryset for passwords to the last 3"""
# 		qs = super(SUserSecurityQuestionInlineAdmin, self).get_queryset(request)
# 		filtered = qs[:3].values_list('id', flat=True)
# 		return qs.filter(id__in=filtered)
...
# class ExtendedSUserPermissionInline(admin.TabularInline):
# 	"""
# 	This class registers the ExtendedUserPermission model with the admin backend
# 	"""
# 	model = ExtendedSUserPermission
...
# 	inlines = [SUserSecurityQuestionInlineAdmin, ExtendedSUserPermissionInline, SUserPasswordInlineAdmin]
...
# 		), ] + super(SUserAdmin, self).get_urls()
...
# 		return super(SUserAdmin, self).add_view(request, form_url, extra_context)
...
# 		return super(SUserAdmin, self).response_add(request, obj, post_url_continue)
...
# 	list_filter = ('date_created',)
# 	ordering = ('-date_created',)
# 	list_display = ('SUser', 'expires_at', 'state', 'date_modified', 'totp_key', 'date_created')
# 	search_fields = ('SUser__name', 'corporate_customer__name', 'totp_key', 'state__name')
