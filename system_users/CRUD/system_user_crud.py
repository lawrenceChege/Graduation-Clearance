"""
This module will hold administration for SUser
"""
...
from SUser.backend.services import RoleService, SUserService, ExtendedSUserPermissionService, PermissionService, \
	RolePermissionService


class SUserAdministration(TransactionLogBase):
	"""
	Administration for SUser
	"""
	...
	def add_custom_role(self, request, **kwargs) -> JsonResponse('data', encoder=DjangoJSONEncoder, safe=False):
		...
		transaction = self.log_transaction(
			transaction_type="Add Custom Role", trace='SUser/add_custom_role', request=request)
		try:
			if not transaction:
	...
	def disable_custom_role(self, request, **kwargs) -> JsonResponse('data', encoder=DjangoJSONEncoder, safe=False):
		...
		"""
		transaction = self.log_transaction(
			transaction_type="Disable Custom Role", trace='SUser/disable_custom_role', request=request)
		try:
			if not transaction:
				return {'code': '800.001.001', 'message': 'Failed to log transaction.'}
	...
	def enable_custom_role(self, request, **kwargs) -> JsonResponse('data', encoder=DjangoJSONEncoder, safe=False):
		...
		"""
		transaction = self.log_transaction(
			transaction_type="Enable Custom Role", trace='SUser/enable_custom_role', request=request)
		try:
			if not transaction:
				return {'code': '800.001.001', 'message': 'Failed to log transaction.'}
			role_id = kwargs.get('role')
	...
	def add_custom_role_permission(
		...
		"""
		transaction = self.log_transaction(
			transaction_type="Add Custom Role Permission", trace='SUser/add_custom_role_permission',
			request=request)
		try:
			if not transaction:
				return {'code': '800.001.001', 'message': 'Failed to log transaction.'}
	...
	def enable_custom_role_permission(
		...
		"""
		transaction = self.log_transaction(
			transaction_type="Enable Custom Role Permission", trace='SUser/enable_custom_role_permission',
			request=request)
		try:
			if not transaction:
				return {'code': '800.001.001', 'message': 'Failed to log transaction.'}
	...
	def disable_custom_role_permission(
		...
		"""
		transaction = self.log_transaction(
			transaction_type="Disable Custom Role Permission", trace='SUser/disable_custom_role_permission',
			request=request)
		try:
			if not transaction:
				return {'code': '800.001.001', 'message': 'Failed to log transaction.'}
	...
	def add_organization_admin(self, request, **kwargs) -> JsonResponse('data', encoder=DjangoJSONEncoder, safe=False):
		...
		"""
		transaction = self.log_transaction(
			transaction_type="Add Organization Admin", trace='SUser/enable_custom_role', request=request)
		try:
			if not transaction:
				return {'code': '800.001.001', 'message': 'Failed to log transaction.'}
			...
			organizational_admin = SUserService().create(
	...
	def extend_user_permission(self, request, **data) -> JsonResponse('data', encoder=DjangoJSONEncoder, safe=False):
		...
		"""
		transaction = self.log_transaction(
			transaction_type="Extend User Permission", trace='SUser/extend_user_permission', request=request)
		try:
			if not transaction:
				return {'code': '800.001.001', 'message': 'Failed to log transaction.'}
			user = data.pop('user')
			...
			SUser = SUserService().get(pk=user)
			if not SUser:
			...
			k['permission'] = permission
			extend_user_permission = ExtendedSUserPermissionService().create(**k)
	...
	def add_organization_user(self, request, **kwargs) -> JsonResponse('data', encoder=DjangoJSONEncoder, safe=False):
		...
		transaction = self.log_transaction(
			transaction_type="Add Organization User", trace='SUser/add_organization_user', request=request)
		try:
			if not transaction:
				return {'code': '800.001.001', 'message': 'Failed to log transaction.'}
	...
	def add_support_user(self, request, **kwargs) -> JsonResponse('data', encoder=DjangoJSONEncoder, safe=False):
		...
		"""
		transaction = self.log_transaction(
			transaction_type="Add Support User", trace='SUser/add_support_user', request=request)
		try:
			if not transaction:
				return {'code': '800.001.001', 'message': 'Failed to log transaction.'}
			branch = kwargs.pop("branch")
	...
	def update_organization_admin(self, request, **kwargs) -> JsonResponse('data', encoder=DjangoJSONEncoder, safe=False):
		...
		"""
		transaction = self.log_transaction(
			transaction_type="Update Organization Admin", trace='SUser/update_organization_admin', request=request)
		try:
			if not transaction:
				return {'code': '800.001.001', 'message': 'Failed to log transaction.'}
			...
			SUser = SUserService().get(pk=org_admin_id)
			if not SUser:
				self.mark_transaction_failed(transaction, code="500.004.002", description="User not found")
				return {"code": "500.004.002", "message": "User not found", "transaction": str(transaction.id)}
			if branch:
			...
			if email:
				if not validate_email(email):
					...
					return {
						...
						'transaction': str(transaction.id)}
				k['email'] = email
			update_organizational_admin = SUserService().update(org_admin_id, **k)
	...
	def update_organization_user(self, request, **data) -> JsonResponse('data', encoder=DjangoJSONEncoder, safe=False):
		...
		"""
		transaction = self.log_transaction(
			transaction_type="Update Organization User", trace='SUser/enable_custom_role_permission',
			request=request)
		try:
			if not transaction:
				return {'code': '800.001.001', 'message': 'Failed to log transaction.'}
			...
			if other_name:
				...
				k['other_name'] = str(other_name).strip().title()
			SUser = SUserService().update(SUser.id, **k)
			if not SUser:
	...
	def update_support_user(self, request, **kwargs) -> JsonResponse('data', encoder=DjangoJSONEncoder, safe=False):
		...
		# TODO: Support staff differentiators?
		transaction = self.log_transaction(
			transaction_type="Update Support User", trace='SUser/update_support_user', request=request)
		try:
			if not transaction:
				return {'code': '800.001.001', 'message': 'Failed to log transaction.'}
			SUser = kwargs.pop("SUser")
			first_name = kwargs.get("first_name")
			last_name = kwargs.get("last_name")
			other_name = kwargs.get("other_name")
			phone_number = kwargs.get("phone_number")
			...
			k['email'] = str(email).strip().lower()
			k['language_code'] = language_code
			user = SUserService().update(pk=SUser, **k)
			if not user:
				self.mark_transaction_failed(
	...
	def get_user(self, request, **kwargs) -> JsonResponse('data', encoder=DjangoJSONEncoder, safe=False):
		...
		:return: JsonResponse
		"""
		transaction = self.log_transaction(transaction_type="Get User", trace='SUser/get_user', request=request)
		try:
			if not transaction:
				return {'code': '800.001.001', 'message': 'Failed to log transaction.'}
			...
			if not SUser:
				self.mark_transaction_failed(transaction, code="500.004.002", description="User not found")
				return {"code": "500.004.002", "message": "User not found ", "transaction": str(transaction.id)}
			extended_permissions = list(ExtendedSUserPermissionService().filter(
				SUser=SUser, state__name='Active').values_list('permission__id', flat=True))
			print(extended_permissions)
			role_permissions = list(RolePermissionService().filter(
				role=SUser.role, state__name='Active').values_list('permission__id', flat=True))
			permissions = list(chain(extended_permissions, role_permissions))
			data = {
				"first_name": SUser.first_name, "last_name": SUser.last_name, "phone_number": SUser.phone_number,
				"email": SUser.email, "branch": {"branch_id":str(SUser.branch.id),"name":SUser.branch.name},
				'role':{"role_id":str(SUser.role.id),"name":SUser.role.name},
	...
	def disable_user(self, request, **kwargs) -> JsonResponse('data', encoder=DjangoJSONEncoder, safe=False):
		...
		"""
		transaction = self.log_transaction(
			transaction_type="Disable User", trace='SUser/disable', request=request)
		try:
			if not transaction:
				return {'code': '800.001.001', 'message': 'Failed to log transaction.'}
			user_id = kwargs.get('SUser')
			if user_id and not validate_uuid4(user_id):
				self.mark_transaction_failed(transaction, code='999.005.006', description='Invalid User')
				return {'code': '999.005.006', 'message': 'SUser not valid', 'transaction': str(transaction.id)}
			active = StateService().get(name="Active")
			user = SUserService().get(pk=user_id, state=active)
			if not user:
				self.mark_transaction_failed(transaction, code='500.004.002', description='User not found')
				return {'code': '500.004.002', 'message': 'User not found', 'transaction': str(transaction.id)}
			disabled = StateService().get(name="Disabled")
			user = SUserService().update(pk=user_id, state=disabled)
			if user:
				self.complete_transaction(transaction, "100.000.000", "Success")
				return {'code': '100.000.000', 'message': 'Success', 'data': {'user': str(user.id)}}
	...
	def enable_user(self, request, **kwargs) -> JsonResponse('data', encoder=DjangoJSONEncoder, safe=False):
		...
		"""
		transaction = self.log_transaction(
			transaction_type="Enable User", trace='SUser/enable_user', request=request)
		try:
			if not transaction:
				return {'code': '800.001.001', 'message': 'Failed to log transaction.'}
			user_id = kwargs.get('SUser')
			if user_id and not validate_uuid4(user_id):
				...
				return {'code': '999.005.006', 'message': 'SUser not valid', 'transaction': str(transaction.id)}
			disabled = StateService().get(name="Disabled")
			user = SUserService().get(pk=user_id, state=disabled)
			if not user:
				self.mark_transaction_failed(transaction, code='500.004.002', description='SUser not found')
				return {'code': '500.004.002', 'message': 'SUser not found', 'transaction': str(transaction.id)}
			active = StateService().get(name="Active")
			user = SUserService().update(pk=user_id, state=active)
			if user:
				self.complete_transaction(transaction, "100.000.000", "Success")
				return {'code': '100.000.000', 'message': 'Success', 'data': {'user': str(user.id)}}
	...
	def change_password(self, request, **kwargs) -> JsonResponse('data', encoder=DjangoJSONEncoder, safe=False):
		...
		"""
		transaction = self.log_transaction(
			transaction_type="Change Password", trace='SUser/change_password', request=request)
		try:
			if not transaction:
				return {'code': '800.001.001', 'message': 'Failed to log transaction.'}
			...
			e_user = SUserService().get(pk=user_id, state=active)
			if not e_user:
				self.mark_transaction_failed(transaction, code='500.004.002', description='SUser not found')
				return {'code': '500.004.002', 'message': 'User not found', 'transaction': str(transaction.id)}
